...
export function createRoomScene(canvas: HTMLCanvasElement): RoomScene {
  ...
  const wallMat  = new THREE.MeshStandardMaterial({ color: 0x0e0e1e, roughness: 0.9 })
  const floorMat = new THREE.MeshStandardMaterial({ color: 0x0d0d1a, roughness: 0.85 })
  const ceilMat  = new THREE.MeshStandardMaterial({ color: 0x080810, roughness: 0.95 })
  const deskMat  = new THREE.MeshStandardMaterial({ color: 0x1a1a2a, roughness: 0.7 })
  ...
  // Lighting
  scene.add(new THREE.AmbientLight(0xa0a0c0, 0.8))

  const overhead = new THREE.PointLight(0xffeedd, 1.0, 18)
  overhead.position.set(0, ROOM_H - 0.3, 0)
  scene.add(overhead)

  const monitorGlow = new THREE.PointLight(0x4a9eff, 1.6, 7)
  monitorGlow.position.set(0, 1.85, -3.2)
  scene.add(monitorGlow)

  const tvGlow = new THREE.PointLight(0x9b59b6, 1.2, 6)
  tvGlow.position.set(3.5, 2.1, -1.0)
  scene.add(tvGlow)

  const posterGlow = new THREE.PointLight(0x00e5a0, 0.9, 5)
  posterGlow.position.set(-3.5, 1.8, 0)
